...
export default function Reports() {
  ...
  const [distribution, setDistribution] = useState<any>({ by_thrust_area: [], by_uom_type: [] });
  const [loading, setLoading] = useState(true);
  const [tab, setTab] = useState<'report' | 'trends' | 'distribution'>('report');

  useEffect(() => {
    Promise.all([reportsApi.getAchievementReport(), reportsApi.getQoQTrends(), reportsApi.getDistribution()])
      .then(([r, q, d]) => { setReport(r); setQoq(q); setDistribution(d); })
        ...
        {[{ k: 'report', l: 'Achievement' }, { k: 'trends', l: 'QoQ Trends' }, { k: 'distribution', l: 'Distribution' }].map((t) => (
          <button key={t.k} onClick={() => setTab(t.k as any)}
            className={`px-4 py-2.5 text-sm font-medium border-b-2 -mb-px transition-colors ${
              tab === t.k ? 'border-zinc-900 text-zinc-900' : 'border-transparent text-zinc-400 hover:text-zinc-600'
            }`}>{t.l}</button>
        ))}
      </div>
          ...
          {tab === 'trends' && (
            <div className="bg-white border border-zinc-200 rounded-xl p-6">
              <ResponsiveContainer width="100%" height={300}>
                <LineChart data={qoq.map(q => ({ ...q, score: Math.round(q.avg_score * 100) }))}>
