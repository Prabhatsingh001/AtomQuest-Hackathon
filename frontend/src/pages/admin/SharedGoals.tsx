...
export default function SharedGoals() {
    ...
    const users = allUsers.filter(x => x.role === 'employee');

    const { data: sharedGoals = [], isLoading: loadingGoals } = useQuery({
        queryKey: ['sharedGoals'],
        queryFn: () => adminApi.getSharedGoals(),
        staleTime: 1000 * 60 * 2,
    });

    const loading = loadingUsers || loadingGoals || submitting;
    ...
    return (
        <div className="max-w-5xl mx-auto space-y-6">
            ...
            <div className="flex flex-col lg:flex-row gap-6 mt-8">
                <div className="w-full lg:w-80 shrink-0 bg-white border border-zinc-200 rounded-xl p-5 self-start shadow-sm">
                ...
                <div className="flex-1 min-w-0 bg-white border border-zinc-200 rounded-xl p-6 self-start shadow-sm">
                    ...
                    <form onSubmit={handleSubmit} className="space-y-4 max-w-xl">
                        ...
                        <div className="pt-4 border-t border-zinc-100">
                            <button type="submit" disabled={submitting || selectedEmployees.length === 0}
                                ...
                                {submitting ? 'Pushing...' : <><Share2 className="w-4 h-4" /> Push to {selectedEmployees.length} Employees</>}
                            </button>
                        </div>
                    </form>
                </div>
            ...
            <div className="mt-12 bg-white border border-zinc-200 rounded-xl overflow-x-auto max-w-full shadow-sm">
                <div className="px-6 py-4 border-b border-zinc-200 min-w-[600px]">
