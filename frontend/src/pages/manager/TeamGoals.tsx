...
export default function TeamGoals() {
    ...
    const handleReturn = async () => {
        if (!selected) return;
        try {
            await approvalsApi.returnSheet(selected.id, returnComment);
                    ...
                    {loading ? [1, 2].map(i => <div key={i} className="bg-zinc-100 rounded-xl h-20 animate-pulse" />) :
                        queue.length === 0 ? (
                            ...
                            <button key={s.id} onClick={() => setSelected(s)}
                                className={`w-full text-left p-4 rounded-xl border text-sm transition-all ${selected?.id === s.id ? 'border-zinc-900 bg-zinc-50 shadow-sm' : 'border-zinc-200 bg-white hover:bg-zinc-50 hover:border-zinc-300'
                                    }`}>
