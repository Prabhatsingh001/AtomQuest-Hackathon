...
export default function CycleManager() {
    ...
    const handleActivate = async () => {
        ...
        try {
            await adminApi.activateCycle(activateId);
        ...
        <div className="max-w-3xl mx-auto space-y-6">
            ...
            <div className="space-y-3">
                {loading ? [1, 2].map(i => <div key={i} className="bg-zinc-100 rounded-xl h-24 animate-pulse" />) :
                    cycles.map((c) => (
                        <div key={c.id} className={`bg-white border rounded-xl p-5 shadow-sm ${c.is_active ? 'border-emerald-300' : 'border-zinc-200'}`}>
